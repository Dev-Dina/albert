...
- `widget.js` loader: < 4 KB minified; widget bundle (React + ReactDOM + chat UI): ≤ 160 KB minified before gzip, ≤ 55 KB gzipped. (Raised from the initial 110 KB / 45 KB target after the React 18 + ReactDOM + chat UI build measured ~145 KB; see Open Risks for the planned follow-up to trim via preact-compat or code-split. The raised ceiling still keeps the bundle small relative to a typical embedded chat widget and is enforced by T088 in CI.)
...
- **React bundle size**: the initial 110 KB / 45 KB-gzipped target proved tight — the first build of React 18 + ReactDOM + the chat UI measured ~145 KB minified. The plan budget has been raised to 160 KB / 55 KB-gzipped with ~10% headroom, enforced by T088 in CI. A bundle-trim follow-up (T095a: preact-compat substitution or further code-split) is scheduled to come back under 110 KB after MVP ships; the raised ceiling is explicitly a temporary accommodation, not a final target.
